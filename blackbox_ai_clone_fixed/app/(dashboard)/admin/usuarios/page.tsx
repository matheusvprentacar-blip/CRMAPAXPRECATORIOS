...
export default function UsuariosPage() {
  ...
  async function handleEditarRole(usuario: Usuario) {
    ...
    try {
      ...
      if (supabase) {
        const { error: dbError } = await supabase.from("usuarios").update({ role: newRole }).eq("id", usuario.id)
        ...
        if (authError) {
          console.error("[ADMIN] Erro ao atualizar Auth metadata:", authError)
          // Continue mesmo se falhar - o role na tabela já foi atualizado
        }
        ...
        toast({
          ...
          description: "O usuário precisa fazer logout e login novamente para as mudanças terem efeito.",
          duration: 5000,
        })
