...
export default function PrecatorioDetailPage({ params }: { params: { id: string } }) {
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from("precatorios").update(updateData).eq("id", params.id)
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {precatorio.pdf_url && (
                <>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => router.push(`/calcular?id=${params.id}`)}>
                    Abrir Calculadora
                  </Button>
                </>
              )}
