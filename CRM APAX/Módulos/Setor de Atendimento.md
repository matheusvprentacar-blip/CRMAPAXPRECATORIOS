...
> [!warning] Importação em lote: valor saía dividido por 1000 (milhar virava centavos)
> Na importação por planilha, valores como `R$ 366.910,00` chegavam à fila como **R$ 366,91** (`3.160 → 3,16`, `208.940 → 208,94` etc.) — o número saía com casas decimais a menos.
> **Causa raiz:** a API `importar-excel` lê a planilha com `XLSX.utils.sheet_to_json(..., { raw: false })`, então o Excel devolve o número **formatado em padrão BR**. Uma célula `366910` sem casas decimais vira a string `"366.910"`. O antigo `parseBRL` só removia os pontos de milhar **quando havia vírgula** (`if (limpo.includes(","))`); sem vírgula, o `parseFloat("366.910")` interpretava o ponto como **separador decimal** → `366.91`.
> **Correção:** `parseBRL` (em `ModalImportacaoLote`) agora também trata o caso sem vírgula: quando o formato é milhar BR bem-formado (1º grupo com 1–3 dígitos e os demais com exatamente 3, ex.: `"366.910"`, `"1.234.567"`), remove os pontos; caso contrário (`"1000.5"`, `"1234.567"`) mantém o ponto como decimal. Também aceita número puro vindo do Excel. Coberto por casos de teste manuais (milhar puro, milhar+decimal, decimal puro, número cru).
