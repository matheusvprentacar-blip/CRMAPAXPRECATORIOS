...
import { ArrowLeft, ArrowRight, Calculator, ShieldCheck } from "lucide-react"
      ...
      <CardHeader className="space-y-3">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle className="text-xl">PSS - Previdência Social</CardTitle>

            </div>
          </div>

          <div className="flex items-center gap-4 rounded-lg border border-border bg-muted/30 p-2 shadow-sm">
            ...
            {!isento && (
              <>
                <div className="h-4 w-px bg-border/60" /> {/* Separator */}
                <div className="flex items-center gap-2 px-2">
                  <Switch
                    id="manual-mode-pss"
                    checked={isManual}
          ...
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2 md:col-span-2">
                <Label className="text-xs uppercase tracking-wide text-muted-foreground">Valor do PSS no Ofício (R$)</Label>
                <Input
                  type="number"
                  step="0.01"
                  placeholder="0,00"
                ...
                <p className="text-xs text-muted-foreground">
                  Base informada no ofício para cálculo de PSS.
                </p>
              </div>

              <div className="rounded-2xl border border-border/60 bg-muted/30 p-4">
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Total PSS atualizado</p>
            ...
            {!isManual ? (
              <div className="rounded-2xl border border-border/60 bg-background/70 p-4 shadow-sm">
                ...
                <div className="mt-4 space-y-3">
                  <div className="flex flex-col gap-2 rounded-xl border border-border/60 bg-muted/30 p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground">1. Correção (IPCA-E)</p>
                      <p className="text-xs text-muted-foreground">
                        {formatarMoeda(pssOficio)} x {fatorIPCA.toFixed(6)}
                      </p>
                    </div>
                    <span className="font-semibold tabular-nums text-blue-700 dark:text-blue-200">
                      {formatarMoeda(pssIPCA)}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 rounded-xl border border-border/60 bg-muted/30 p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatarMoeda(pssOficio)} x {fatorIpca2025.toFixed(6)}
                      </p>
                    </div>
                    <span className="font-semibold tabular-nums text-purple-700 dark:text-purple-200">
                      {formatarMoeda(pssIpca2025)}
                    </span>
                  </div>

                  <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/40 p-3">
                    <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total PSS atualizado</span>
              ...
              <div className="space-y-2 rounded-2xl border border-amber-200/60 bg-amber-50/60 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
                <Label className="text-sm font-semibold text-amber-800 dark:text-amber-200">Valor PSS Manual (Total)</Label>
                <Input
                  type="number"
                  className="h-11 rounded-xl border-amber-300 text-base focus-visible:ring-amber-500 dark:border-amber-800"
                  value={pssManualValor}
                  onChange={(e) => setPssManualValor(Number.parseFloat(e.target.value) || 0)}
                />
                <p className="text-xs text-amber-700 dark:text-amber-300">
                  Você está definindo o valor final manualmente.
                </p>
              </div>
            )}
          </div>
