          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <p className="font-medium">{formatarMoeda(resumo.valor_atualizado ?? etapaAtualizacao.valor_atualizado)}</p>
              </div>
              <div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              {!isentoPss && (
                <div>
                  ...
                  <p className="font-medium text-red-600">{formatarMoeda(resumo.pss_valor ?? etapaPss.pss_valor)}</p>
                </div>
              )}
              {isentoPss && (
                <div>
              ...
              <div>
                ...
                <p className="font-medium text-red-600">
                  {resumo.honorarios_percentual?.toFixed(2) || etapaHonorarios.honorarios_percentual?.toFixed?.(2) || "0.00"}% · {formatarMoeda(resumo.honorarios_valor ?? etapaHonorarios.honorarios_valor)}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Adiantamento</p>
                <p className="font-medium text-red-600">
                  {resumo.adiantamento_percentual?.toFixed(2) || etapaHonorarios.adiantamento_percentual?.toFixed?.(2) || "0.00"}% · {formatarMoeda(resumo.adiantamento_valor ?? etapaHonorarios.adiantamento_valor)}
                </p>
              </div>
            </div>
          </div>
