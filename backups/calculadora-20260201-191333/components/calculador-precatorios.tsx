...
import { Check, RotateCcw, Eye, ChevronsRight } from "lucide-react"
  ...
  const finalizarCalculo = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (histError) console.error("Erro ao salvar hist?rico:", histError)
  ...
  return (
    <div className="relative space-y-6">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-24 right-0 h-64 w-64 rounded-full bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.18),transparent_65%)] blur-2xl" />
        <div className="absolute -bottom-32 left-0 h-72 w-72 rounded-full bg-[radial-gradient(circle_at_bottom,rgba(14,116,144,0.18),transparent_70%)] blur-2xl" />
      </div>

      <Card className="relative border border-border/60 bg-card/80 backdrop-blur-xl shadow-[0_30px_80px_-55px_rgba(15,23,42,0.6)] sticky top-4 z-40">
        <div className="relative border-b border-border/60 bg-gradient-to-r from-background via-background to-muted/40">
          ...
          <div className="p-6">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground">Calculadora</p>
                <h2 className="text-3xl font-semibold tracking-tight text-foreground">
                  Calculadora de Precatórios
                </h2>
                <p className="text-sm text-muted-foreground">
                  Sistema completo de cálculo com validação por etapas
                </p>
                {precatorioData && (
              ...
              {precatorioId && (
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold shadow-sm transition-all ${showPdfSide
                  ...
                  <button
                    ...
                    disabled={saving}
                    className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/80 px-4 py-2 text-sm font-semibold text-muted-foreground shadow-sm transition hover:bg-muted/60 hover:text-foreground disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-secondary/80 px-4 py-2 text-sm font-semibold text-secondary-foreground shadow-sm transition hover:bg-secondary/70 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    disabled={saving || resultadosEtapas.filter(Boolean).length < 6}
                    className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-md shadow-primary/20 transition hover:shadow-primary/30 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="px-6 py-5 border-b border-border/60 bg-gradient-to-r from-muted/40 via-background to-muted/20">
        ...
        <div className="p-6">
          <div className="space-y-6">
            <div className="rounded-2xl border border-border/60 bg-muted/30 p-4 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Navegue pelo fluxo</p>
                </div>
                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-w-[180px] items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all ${isActive
                        ? "border-primary/40 bg-primary/10 shadow-sm"
                        : isCompleted
                          ? "border-emerald-200/70 bg-emerald-50/60 text-emerald-900 dark:border-emerald-800/60 dark:bg-emerald-950/30"
                          : "border-border/60 bg-background/70 hover:bg-muted/50"
                        }`}
                    >
                      <span
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-xs font-semibold transition-colors ${isActive
            ...
            <div className={`grid gap-6 ${showPdfSide ? "lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)]" : ""}`}>
              ...
              {showPdfSide && (
                <div className="h-[calc(100vh-12rem)] rounded-2xl border border-border/60 bg-muted/30 overflow-hidden shadow-sm xl:sticky xl:top-28 transition-all duration-300 animate-in fade-in slide-in-from-right-10">
                  <div className="flex items-center justify-between px-3 py-2 bg-background/80 backdrop-blur border-b border-border/60">
                    <span className="text-sm font-medium">Documento do Precatório</span>
                    <button
      ...
      <AlertDialog open={showResetDialog} onOpenChange={setShowResetDialog}>
        <AlertDialogContent className="bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-50 border border-border shadow-xl">
          <AlertDialogHeader>
