...
import { UploadEscrituraModal } from "@/components/precatorios/upload-escritura-modal"
...
interface PrecatEscritura {
  ...
  responsavel_escrituras?: { nome: string } | null
  responsavel_nome?: string
  created_at: string
  status_kanban?: string
  status_escrituras?: string
}
...
const escriturasGridRows = "auto-rows-[388px]"
const escriturasCardHeight = "h-[388px]"

const STATUS_ESCRITURAS_LABELS: Record<string, string> = {
...
const STATUS_BADGE_CLASSES: Record<string, string> = {
  nao_iniciado: "border-default-300/70 bg-default-100/50 text-foreground/80",
  em_andamento: "border-primary/40 bg-primary/10 text-primary",
  pendente_assinatura: "border-warning/40 bg-warning/10 text-warning",
  concluido: "border-success/40 bg-success/10 text-success",
}

function isMissingEscriturasSchema(error: unknown): boolean {
  ...
  return (
    ...
    blob.includes("observacoes_escrituras") ||
...
export default function GestaoEscriturasPage() {
  const router = useRouter()
  const [precatorios, setPrecatorios] = useState<PrecatEscritura[]>([])
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        ...
        .select(
          `
            ...
            status_kanban,
            status_escrituras,
            responsavel_escrituras:responsavel_escrituras_id(nome)
          `
        )
        .or(`status_kanban.eq.escrituras,responsavel_escrituras_id.eq.${currentUser.id}`)
        .order("created_at", { ascending: true })

      if (fetchError) {
        if (isMissingEscriturasSchema(fetchError)) {
          const { data: fallbackData, error: fallbackError } = await supabase
            ...
            .eq("status_kanban", "escrituras")
            .order("created_at", { ascending: true })

          if (fallbackError) {
            console.error("[Escrituras] Erro ao carregar fallback:", fallbackError)
            setError("Banco desatualizado para escrituras. Aplique as migrations 221, 222 e 223 no Supabase.")
            setLoading(false)
            return
          }

          const fallbackFormatted = (fallbackData || []).map((p: any) => ({
            ...p,
            responsavel_nome: "Sem responsável",
            status_escrituras: "nao_iniciado",
          }))

          setPrecatorios(fallbackFormatted)
          setError("Banco desatualizado para escrituras. Aplique as migrations 221, 222 e 223 no Supabase.")
          setLoading(false)
          return
        }

        console.error("[Escrituras] Erro ao carregar fila:", fetchError)
        setError("Erro ao carregar fila de escrituras.")
        setLoading(false)
        return
      }
      ...
      const formatted = (data || []).map((p: any) => ({
        ...p,
        responsavel_nome: p.responsavel_escrituras?.nome || "Sem responsável",
      }))
      ...
      console.error("[Escrituras] Erro inesperado:", err)
  ...
  const handleAbrir = (id: string) => {
    router.push(`/precatorios/detalhes?id=${id}&tab=escrituras`)
  }
  ...
  const resumo = useMemo(() => {
    ...
    const emAndamento = precatorios.filter((p) => p.status_escrituras === "em_andamento").length
    const concluidos = precatorios.filter((p) => p.status_escrituras === "concluido").length
    return { total, semResponsavel, emAndamento, concluidos }
  }, [precatorios])
  ...
  const precatoriosFiltrados = useMemo(() => {
    ...
    return precatorios.filter((p) => {
      const statusAtual = p.status_escrituras || "nao_iniciado"
  ...
  if (loading) {
    return (
      <div className="escrituras-revamp mx-auto max-w-7xl space-y-4 p-6">
  ...
  return (
    <div className="escrituras-revamp container mx-auto max-w-[1400px] space-y-4 p-4 md:space-y-5 md:p-6">
      <Card className={`${glassCard} escrituras-hero-card`}>
        <CardContent className="space-y-5 p-5 md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Gestão de Escrituras</h1>
              <p className="max-w-3xl text-sm text-foreground/70 md:text-base">
                Fluxo de escrituras com visual operacional, análise rápida e ação direta de anexar documento.
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Card className="border border-default-200/75 bg-content1/90">
              <CardContent className="flex items-center gap-3 p-3">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-foreground/65">Em andamento</p>
                  <p className="text-xl font-semibold">{resumo.emAndamento}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border border-default-200/75 bg-content1/90">
              <CardContent className="flex items-center gap-3 p-3">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-foreground/65">Concluídos</p>
                  <p className="text-xl font-semibold">{resumo.concluidos}</p>
                </div>
              </CardContent>
            </Card>
      ...
      <Card className={`${glassCard} escrituras-search-card`}>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_220px_auto]">
            ...
            <Select value={statusFiltro} onValueChange={setStatusFiltro}>
              ...
              <SelectContent>
                ...
                {Object.entries(STATUS_ESCRITURAS_LABELS).map(([status, label]) => (
          ...
          <CardContent className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center text-foreground/70">
            ...
            <p className="text-sm">
              {precatorios.length === 0
                ? 'Processos marcados para "Escrituras" aparecerão aqui.'
                : "Ajuste o termo da busca ou o status selecionado para ampliar os resultados."}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 ${escriturasGridRows}`}>
          {precatoriosFiltrados.map((p, index) => {
            const statusKey = p.status_escrituras || "nao_iniciado"
            const statusLabel = STATUS_ESCRITURAS_LABELS[statusKey] || p.status_escrituras || "Não iniciado"
            const statusClass = STATUS_BADGE_CLASSES[statusKey] || STATUS_BADGE_CLASSES.nao_iniciado
            ...
            return (
              <SpotlightCard key={p.id} className="h-full w-full rounded-[1.2rem]" spotlightColor="hsl(var(--primary) / 0.26)">
                <Card
                  ...
                  tabIndex={0}
                  className={[
                    `${escriturasCardHeight} w-full min-w-0 rounded-[1.2rem] border border-default-200/75 bg-content1/92`,
                    "flex flex-col",
                ...
                >
                  ...
                  <div className="mt-auto grid grid-cols-2 gap-2 p-3">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="escrituras-attach-btn rounded-full"
                      onClick={(event) => {
                        event.stopPropagation()
