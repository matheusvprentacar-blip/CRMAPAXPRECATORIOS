...
export default function TelemetriaPage() {
  ...
  const loadTelemetry = useCallback(async (isRefresh = false) => {
    ...
    const { data, error: queryError } = await supabase
      .from("telemetria_uso")
      .select("id, user_id, session_id, event_type, source, event_data, occurred_at, usuario:usuarios(nome, email, role)")
      .order("occurred_at", { ascending: false })
      .limit(400)

    if (queryError) {
      setError(queryError.message || "Não foi possível carregar os eventos de telemetria.")
      setEvents([])
      setLoading(false)
      setRefreshing(false)
      return
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Observação: esta tela analisa até <span className="font-medium">400 eventos</span> (limite do carregamento atual).
            </p>
          </div>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="rounded-lg border border-border/60 bg-muted/30 p-3">
                ...
                <div className="mt-1 text-sm">
                  Para ficar ainda mais preciso, aumente o limite do select (ou use paginação) e inclua{" "}
                  <span className="font-medium">session_started/session_ended</span> sempre.
                </div>
              </div>
            </CardContent>
          </Card>
