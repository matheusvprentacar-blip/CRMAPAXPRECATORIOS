...
BEGIN
    ...
    WHERE p.deleted_at IS NULL
      AND (
        ...
        OR (
          ...
          AND (
            NULLIF(to_jsonb(p) ->> 'dono_usuario_id', '')::UUID = v_user_id
  ...
  ranked AS (
    SELECT
      n.*,
      ROW_NUMBER() OVER (
        PARTITION BY n.id_unico
        ORDER BY n.ref_data DESC NULLS LAST, n.id DESC
      ) AS rn
    FROM normalized n
  ),
  aggregated AS (
  ...
  SELECT
    ...
    r.status_atual AS ultimo_status,
    COALESCE(r.valor_atualizado, 0)::NUMERIC AS ultimo_precatorio_valor
  FROM aggregated a
  LEFT JOIN ranked r
    ON r.id_unico = a.id_unico
   AND r.rn = 1
