...
file_path = r"c:\Users\User\Desktop\crmapaxprecatorios\app\(dashboard)\precatorios\page.tsx"

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Add import
import_stmt = 'import "./precatorios-layout.css"\n'
if import_stmt not in content:
    content = content.replace('import { SearchBar } from "@/components/precatorios/search-bar"', 
                              'import "./precatorios-layout.css"\nimport { SearchBar } from "@/components/precatorios/search-bar"')

# Ensure Search icon is imported
if 'Search' not in content[:1000] and 'import { ' in content:
    content = content.replace('import { Plus, Trash2, X, FileJson, Loader2, Filter, FileText, MoreVertical, LayoutGrid, List }',
                              'import { Plus, Trash2, X, FileJson, Loader2, Filter, FileText, MoreVertical, LayoutGrid, List, Search }')

# The replacement targets from `return (` inside PrecatoriosPage to the end `}` closing the page.
# Finding where `return (` starts after checking loading:
...
parts = content.split("  if (loading && !initialized) {")
before_return = parts[0] + "  if (loading && !initialized) {" + parts[1].split("\n  return (")[0]
    ...
    <div className="page">
      ...
      <div className="toolbar">
        <div className="toolbar-row">
          ...
          {/* Advanced Filters keeps its internal button, we let it render alongside our inline controls */}
          <AdvancedFilters
            filtros={filtros}
      ...
      {/* Selection bar (quando há seleção) */}
      {selectedIds.size > 0 && (
        ...
        <>
          {viewMode === "cards" ? (
            ...
            <div className="cards-grid">
              {precatorios.map((precatorio, index) => {
                ...
                const initials = (precatorio.credor_nome || precatorio.titulo || "P").substring(0, 2).toUpperCase()
                const responsavelNome = precatorio.responsavel_nome || precatorio.responsavel_calculo_nome || "Não atribuído"
                const respInitials = responsavelNome !== "Não atribuído" ? responsavelNome.substring(0, 2).toUpperCase() : "—"
          ...
          <div className="pagination">
            ...
            <div className="pagination-controls">
              ...
              <span className="mx-2 text-sm font-medium text-[var(--text-muted)]">
