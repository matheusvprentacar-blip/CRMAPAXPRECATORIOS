-- Regras de Duplicidade por Dono (sem apagar registros)
-- Objetivo:
...
-- 1) Remover indices UNIQUE antigos que bloqueiam duplicidade do mesmo dono
DROP INDEX IF EXISTS public.idx_unique_numero_precatorio;
DROP INDEX IF EXISTS public.idx_unique_numero_processo;

-- 2) Manter indices simples para performance de busca
...
-- Diagnostico (opcional): mostra conflitos de dono para o mesmo numero
-- SELECT numero_processo, array_agg(DISTINCT COALESCE(dono_usuario_id, criado_por)) AS owners
-- FROM public.precatorios
-- WHERE numero_processo IS NOT NULL AND numero_processo <> '' AND deleted_at IS NULL
-- GROUP BY numero_processo
-- HAVING COUNT(DISTINCT COALESCE(dono_usuario_id, criado_por)) > 1;

-- SELECT numero_precatorio, array_agg(DISTINCT COALESCE(dono_usuario_id, criado_por)) AS owners
-- FROM public.precatorios
-- WHERE numero_precatorio IS NOT NULL AND numero_precatorio <> '' AND deleted_at IS NULL
-- GROUP BY numero_precatorio
-- HAVING COUNT(DISTINCT COALESCE(dono_usuario_id, criado_por)) > 1;
