-- =====================================================
-- SCRIPT 99: Melhoria na Busca (Ignorar Acentos) - CORRIGIDO
-- =====================================================
-- Descrição: Habilita a extensão 'unaccent' e atualiza a função
--            de busca global para ignorar acentos e case.
--            Inclui DROP explícito para evitar erro de assinatura.
-- =====================================================

-- 1. Habilitar extensão unaccent (se permissão permitir)
CREATE EXTENSION IF NOT EXISTS unaccent;

-- 2. Drop da função antiga com a assinatura EXATA da versão anterior (Baseado no hint do erro)
DROP FUNCTION IF EXISTS public.buscar_precatorios_global(text,text[],uuid,uuid,text[],text[],text[],text[],date,date,date,date,numeric,numeric,boolean,boolean);

-- 3. Atualizar função buscar_precatorios_global
