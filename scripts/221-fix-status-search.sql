-- =====================================================
-- SCRIPT 221: Fix status column type in search
-- =====================================================
-- Resolve o erro de "character varying does not match expected type text in column 16"
-- =====================================================

-- 1. Remover assinatura anterior
DROP FUNCTION IF EXISTS public.buscar_precatorios_global(
  text, text[], uuid, text, uuid, text[], text[], text[], text[],
  date, date, date, date, numeric, numeric, numeric, numeric, numeric, numeric,
  boolean, boolean, boolean, boolean, boolean
);

-- 2. Criar funcao com cast ::TEXT na coluna status (coluna 16)
...
BEGIN
  ...
  SELECT 
    ...
    p.valor_principal,
    p.valor_atualizado,
    p.status::TEXT,           -- <-- AQUI ESTA A CORRECAO (::TEXT)
    p.urgente,
    p.created_at,
    p.criado_por,
    p.responsavel,
...
'Busca global de precatorios (COM CAST ::TEXT) sem busca por operador no termo rapido';
